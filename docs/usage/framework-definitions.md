...
lerd never creates an `app_file` that does not exist yet. The application writes it during its own install, and to a framework an empty one is not a blank slate but a claim that it is already configured: TYPO3 serves its installer while `config/system/settings.php` is absent and fails outright once an empty one exists. `lerd env` says it is skipping the file and leaves it alone, and `lerd site:doctor` points at the framework's own install rather than at `lerd env`. Once the installer has written it, `lerd env` wires the project's services into it as usual.
